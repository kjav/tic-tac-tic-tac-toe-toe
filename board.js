...
function Board(cells) {
    ...
    */
    ...
    this.__repr__ = function() {
        return "<Board at layer {}>".replace(/{}/g, this.layer);
    };
    ...
    this.draw_board = function(active_coordinates) {
        ...
        var is_active = active_coordinates != null;
        if (is_active)
            ...
            for (var i = 0; i < this.rows; i++) {
                ...
                for (var j = 0; j < this.cols; j++) {
                    ...
                    if (j > 0) { //Only after first column
                        ...
                        for (var line in lines_of_substr)
                            substr += "|{}\n".replace(/{}/g, line);
                    }
                ...
                for (var k = 0; k < result_rows[0].length; k++) {
                    for (var r in result_rows)
                        result += result_rows[r][k];
    ...
    this.__str__ = function() {
        return this.draw_board(null)
    };

    this.__getitem__ = function(pos) {
    ...
    this.check_winner = function() {
        ...
        */
        ...
        for (var i = 0; i < rows.length; i++) {
            if (rows[i].filter(c => (c.owner == rows[i][0].owner) && (c.owner != null)).length == this.cols)
                return r[i][0].owner;
        }
        var cols = this.get_cols();
        console.log(cols);
        for (var i = 0; i < cols.length; i++) {
            if (cols[i].filter(c => (c.owner == cols[i][0].owner) && (c.owner != null)).length == this.rows)
                return cols[i][0].owner;
        }
        var diags = this.get_diags();
        for (var i = 0; i < diags.length; i++) {
            if (diags[i].filter(c => (c.owner == diags[i][0].owner) && (c.owner != null)).length == this.rows)
